...
async function loadFFmpeg() {
  ...
  // اگر UMD هنوز آماده نیست، پویا لود کن
  if (typeof window.FFmpeg === 'undefined' || typeof window.FFmpegUtil === 'undefined') {
    await new Promise((resolve, reject) => {
      const add = (src) => new Promise((res, rej) => {
        const s = document.createElement('script');
      ...
      add('https://unpkg.com/@ffmpeg/ffmpeg@0.12.10/dist/umd/ffmpeg.js')
        .then(() => add('https://unpkg.com/@ffmpeg/util@0.12.10/dist/umd/index.js'))
        .then(resolve).catch(reject);
    });
  }
  ...
  // بارگذاری core/wasm با روش توصیه‌شده (سازگار با نبود crossOriginIsolated)
